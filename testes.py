import agenda

# Testes tarefa 3, função horaValida()
print("-----------------------------------")
print("Testes função horaValida()")
print(agenda.horaValida("000000")==False)
print(agenda.horaValida("aaaaaa")==False)
print(agenda.horaValida("0000")==True)
print(agenda.horaValida("2359")==True)
print(agenda.horaValida("2400")==False)
print(agenda.horaValida("12:30")==False)
print(agenda.horaValida("1135")==True)
print("-----------------------------------")

# Testes tarefa 4, função dataValida()
print("-----------------------------------")
print("Testes função dataValida()")
print(agenda.dataValida("20042020")==True)
print(agenda.dataValida("33042020")==False)
print(agenda.dataValida("30022020")==False)
print(agenda.dataValida("20132020")==False)
print(agenda.dataValida("31042020")==False)
print(agenda.dataValida("100220")==False)
print(agenda.dataValida("472928828")==False)
print("-----------------------------------")

#Testes tarefa 5, função projetoValido()
print("-----------------------------------")
print("Testes função projetoValido()")
print(agenda.projetoValido("+Oi tudo bom")==True)
print(agenda.projetoValido("+2")==True)
print(agenda.projetoValido("+")==False)
print(agenda.projetoValido("aapokdap")==False)
print(agenda.projetoValido("")==False)
print("-----------------------------------")

#Testes tarefa 6, função contextoValido()
print("-----------------------------------")
print("Testes função contextoValido()")
print(agenda.contextoValido("@casa")==True)
print(agenda.contextoValido("@apsokdapsdka")==True)
print(agenda.contextoValido("")==False)
print(agenda.contextoValido("@")==False)
print(agenda.contextoValido("+38")==False)
print("-----------------------------------")

#Testes função soLetras()
print("-----------------------------------")
print(agenda.soLetras("apskodsapdk3232poskpoaksd")==False)
print(agenda.soLetras(" ")==False)
print(agenda.soLetras("aaposdk")==True)
print(agenda.soLetras("912839")==False)
print(agenda.soLetras("*¨*(APSOKD")==False)
print(agenda.soLetras("A")==True)
print("-----------------------------------")

#Testes tarefa 7, função prioridadeValida()
print("-----------------------------------")
print(agenda.prioridadeValida("(a)")==True)
print(agenda.prioridadeValida("(A)")==True)
print(agenda.prioridadeValida("ASPODKAPODK")==False)
print(agenda.prioridadeValida("( )")==False)
print(agenda.prioridadeValida("()")==False)
print(agenda.prioridadeValida("(7)")==False)
print(agenda.prioridadeValida("A") == False)
print(agenda.prioridadeValida("") == False)
print("-----------------------------------")